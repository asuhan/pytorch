...
static void eraseOutput(Node* node, int output_nr, Gradient& gradient) {
  ...
  gradient.df_input_captured_outputs.erase(gradient.df_input_captured_outputs.begin() + df_input_captured_outputs_idx);
  for (uint32_t i = 0; i < gradient.df_input_captured_outputs.size(); i++) {
  ...
  for (auto it = gradient.df->nodes().begin(), end = gradient.df->nodes().end(); it != end;
       ++it) {
...
void XlaRemoveUnused(Gradient& gradient) {
  for (auto it = gradient.f->nodes().begin(), end = gradient.f->nodes().end(); it != end;
       ++it) {
